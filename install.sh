...
SRC_ADVISORY_BIN="$STACK_ROOT/bin/apx-advisory"
...
RUNTIME_ADVISORY_BIN="$RUNTIME_BIN_DIR/apx-advisory"
...
sync_runtime() {
  ...
  if [[ "$FROM_PAYLOAD" == "1" ]]; then
    ...
    install_bin "$SRC_ADVISORY_BIN" "$RUNTIME_ADVISORY_BIN" "runtime advisory validator"
